...
export const MobileBottomNav: React.FC = () => {
  ...
  return (
    <nav
      className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-200/80 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]"
      ...
      <div className="grid grid-cols-5 h-14 max-w-lg mx-auto px-1">
        {currentTabs.map((tab, idx) => {
          ...
          const isActive =
            ...
            (tab.to === '/tim-phong' && (location.pathname === '/tim-phong' || location.pathname === '/tim-kiem'));
          return (
            <Link
              key={`${tab.to}-${idx}`}
              to={tab.to}
